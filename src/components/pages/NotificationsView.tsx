...
export interface Notification {
  id: number;
  type: 'token' | 'template' | 'system' | 'welcome';
  title: string;
  desc: string;
  time: string;
  read?: boolean;
...
const NotificationsView = ({ user, notifications, onMarkAllRead, onClearAll }: NotificationsViewProps) => {
  ...
  const getIconStyle = (type: string) => {
    switch (type) {
      ...
      case 'welcome': return 'bg-gradient-to-br from-emerald-400 to-teal-500 text-white shadow-md shadow-emerald-400/20';
      default: return 'bg-gradient-to-br from-slate-400 to-slate-600 text-white shadow-md shadow-slate-400/20';
    }
  };
