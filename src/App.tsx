...
const PaymentProcessingView = ({ setView, buyTokens, selectedPack, setLastPurchase, setSelectedPack, currentUser, selectedPaymentMethod }: any) => {
  useEffect(() => {
      ...
      try {
        const response = await fetch('http://localhost:5000/api/payment', {
          ...
          body: JSON.stringify({
            ...
            tokensAdded: selectedPack?.tokens || 0,
            price: selectedPack?.price,
            paymentMethod: selectedPaymentMethod,
            packageName: selectedPack?.name || selectedPack?.title
          })
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.message || 'Pembayaran gagal');
        
        buyTokens(data.amount);
        setLastPurchase({
          amount: data.amount,
...
const LoginView = ({ setView, onLoginSuccess }: { setView: (v: string) => void, onLoginSuccess?: (user: any) => void }) => {
  ...
  return (
    <section className="min-h-[calc(100vh-80px)] py-12 flex items-center justify-center px-6 bg-gradient-to-b from-white via-stone-50 to-stone-100 dark:from-stone-900 dark:via-stone-900 dark:to-stone-950 relative overflow-hidden">
      ...
      >
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-lg text-xs font-semibold text-red-500">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mb-6">
          <div className="space-y-1.5">
            ...
            <input 
              ...
              required
              className="w-full bg-white dark:bg-stone-700 border border-stone-300 dark:border-stone-600 rounded-lg py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary dark:text-white transition-all" 
            />
          </div>
          <div className="space-y-1.5">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-brand-primary text-white py-2.5 rounded-lg font-bold text-sm hover:bg-amber-600 transition-all mb-6 disabled:opacity-50"
          >
            {loading ? 'Masuk...' : 'Masuk'}
          </button>
        </form>
...
const SignupView = ({ setView, onLoginSuccess }: { setView: (v: string) => void, onLoginSuccess?: (user: any) => void }) => {
      ...
      >
        ...
        <form className="space-y-4 mb-6" onSubmit={handleSubmit}>
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-brand-primary text-white py-2.5 rounded-lg font-bold text-sm hover:bg-amber-600 transition-all mb-6 mt-4 disabled:opacity-50"
          >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-stone-200 dark:border-stone-700">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-black text-stone-900 dark:text-white">{currentUser?.business_name || 'Akun Pengguna'}</p>
              <p className="text-[10px] font-bold text-stone-400">{currentUser ? `Saldo: ${tokenBalance} Token` : 'Paket Gratis'}</p>
            </div>
  ...
  const buyTokens = (amount: number) => {
    ...
    if (currentUser) {
      setCurrentUser((prev: any) => ({
        ...prev,
        token_balance: prev.token_balance + amount
      }));
    }
  };
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'payment-processing':
        return <PaymentProcessingView setView={setView} buyTokens={buyTokens} selectedPack={selectedPack} setLastPurchase={setLastPurchase} setSelectedPack={setSelectedPack} currentUser={currentUser} selectedPaymentMethod={selectedPaymentMethod} />;
