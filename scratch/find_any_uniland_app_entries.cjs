...
console.log('Searching all history folders for any App.tsx in uni-landfarm...');
...
try {
  ...
  for (const folder of folders) {
    ...
    if (fs.existsSync(entriesPath)) {
      ...
      if (data.resource) {
        ...
        if (resLower.includes('uni-landfarm') && (resLower.includes('app.tsx') || resLower.includes('app.js'))) {
  ...
  console.log(`Total matching folders found: ${count}`);
} catch (err) {
